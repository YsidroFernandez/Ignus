...
@Component({
  ...
  templateUrl: './typecontact.component.html',
  styleUrls: ['./typecontact.component.scss']
})
export class TypeContactComponent implements OnInit {
  ...
  new: any;
  faEdit = faEdit;
  modalTitle: string = "Tipo de Contacto";
  modalIcon: string = "fa fa-plus";
  modalName: any;
  modalTemplate: any;
  submitType: string = "Save";
  disabled: boolean;
  // It maintains contacts form display status. By default it will be false.
  ...
  getListContacts() {
    this.globalService.getModel("/api/typeContact").then(
      result => {
        console.log(result);
  ...
  apiAction() {
    ...
    if (this.submitType === "create") {
      ...
      this.globalService.addModel(this.new, "/api/typeContact").then(
        result => {
          console.log(result);
          if (result["status"]) {
      ...
      this.globalService
        .updateModel(this.contact.id, this.new, "/api/typeContact")
        .then(
          result => {
            if (result["status"]) {
  ...
  open(content, action, index: number) {
    ...
    this.contact = Object.assign({}, this.contacts[this.selectedRow]); //se coloca el indice en el arreglo general de servicios para obtener el servicio en especifico
...
onDelete(index: number) {
    ...
    this.coolDialogs.confirm('Esta seguro que desea eliminar?') //cooldialog es un componentes para dialogos simples solo establecemos un titulo lo demas viene por defecto 
        .subscribe(res => {
            if (res) {
                console.log(res);
                this.globalService.removeModel(this.contact.id, "/api/typeContact")
                    .then((result) => {
                        console.log(result);
                        if (result['status']) {
